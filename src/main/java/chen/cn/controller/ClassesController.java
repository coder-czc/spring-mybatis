...
@RequestMapping("/classes")  //这个注解表示控制器的映射的地址为服务器地址+admin
...
public class ClassesController {
    ...
    private ClassesService classesService;  //这是与上面注入的业务实现类的接口
    ...
    public ServerResponse deleteByPrimaryKey(HttpServletRequest request, HttpServletResponse response,@RequestBody JSONObject object) throws IOException {
        ...
        String[] str = id.split(",");
        for (String s : str) {
            i=  classesService.deleteByPrimaryKey(Integer.parseInt(s));
    ...
    public ServerResponse deleteall(String id){
        ...
        if(deleteids !=null && deleteids.length>0){
            ...
            if(classesService.deleteByList(deleteids)>0){
    ...
    @ResponseBody
    public ServerResponse  insertSelective(Classes record, HttpServletRequest request, HttpServletResponse response){
        System.out.println(record.getName());
        if(classesService.insert(record)>0){
    ...
    public ServerResponse   selectByPrimaryKey(Integer id, HttpServletRequest request, HttpServletResponse response){
        ...
        if(classes != null){
            return ServerResponse.createBySuccess(1,classes);
        }else{
    ...
    @ResponseBody
    public ServerResponse updateByPrimaryKeySelective(Classes record, HttpServletRequest request, HttpServletResponse response){
        System.out.println("d:"+record.getId()+','+record.getName());
        if(classesService.updateByPrimaryKeySelective(record)>0){
    ...
    @ResponseBody
    public void  updateByPrimaryKey(Classes record, HttpServletRequest request, HttpServletResponse response){}
    ...
    public ServerResponse   listAll( HttpServletRequest request, HttpServletResponse response){
        ...
        List<Classes> classess=classesService.selectAll();
        if(classess.size()>0){
            return ServerResponse.createBySuccess(classess.size(),classess);
        }else{
            return ServerResponse.createByErrorMessage("没有找到用户");
        }
    }
